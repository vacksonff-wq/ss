...
const incomingEl = $('#incoming');
  ...
  async function getLocalStream() {
    ...
    try {
      ...
      const t = localStream.getAudioTracks && localStream.getAudioTracks()[0];
  ...
  function updateMicUI() {
    ...
    if (micMuted) {
      ...
      micToggleBtn.textContent = '🔇 میوت';
    } else {
      micToggleBtn.classList.remove('muted');
      micToggleBtn.textContent = '🎙️ میکروفن';
    }
    micToggleBtn.disabled = !peer; // disable if no active call
  }
...
function showCallBar(name) {
  ...
  if (micToggleBtn) {
    micToggleBtn.addEventListener('click', async () => {
      try {
        if (!peer) return; // فقط وقتی تماس فعال است
